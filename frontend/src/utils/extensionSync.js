/**
 * Syncs auth state with the FlowPilot AI Chrome extension.
...
function sendToExtension(message) {
  if (!EXTENSION_ID) {
    console.warn('[FlowPilot] VITE_EXTENSION_ID not set — extension sync disabled');
    return;
  }
  if (typeof chrome === 'undefined' || !chrome?.runtime?.sendMessage) {
    console.warn('[FlowPilot] chrome.runtime not available — not running in Chrome');
    return;
  }
  try {
    chrome.runtime.sendMessage(EXTENSION_ID, message, (response) => {
      if (chrome.runtime.lastError) {
        console.warn('[FlowPilot] Extension sync failed:', chrome.runtime.lastError.message);
      } else {
        console.log('[FlowPilot] Extension sync response:', response);
      }
    });
  } catch (err) {
    console.warn('[FlowPilot] Extension sync error:', err);
  }
}
